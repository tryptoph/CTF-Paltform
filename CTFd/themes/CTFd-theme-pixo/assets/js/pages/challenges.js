  ...
  return fetch('/api/v1/users/me/solves')
    ...
    .then(function(response) {
      if (response.success) {
        ...
        // Clear existing solved indicators first (to prevent duplicates)
        document.querySelectorAll(".corner-button-check").forEach(el => el.remove());
        ...
        for (let i = 0; i < solves.length; i++) {
          ...
          // Add check icon
          if (btn.find('.corner-button-check').length === 0) {
            btn.prepend("<i class='fas fa-check corner-button-check'></i>");
          }
          
          console.log("Marked challenge " + challengeId + " as solved");
        }
      }
    })
    .catch(error => {
