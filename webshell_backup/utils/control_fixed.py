...
class ControlUtil:
    """Utility class for controlling WebShell containers - aligned with CTFd-whale approach"""
    
    @staticmethod
    def try_add_container(user_id, image_id):
        """
        Create a new WebShell container - similar to CTFd-whale approach
    ...
    def _create_container(container):
        """
        Create a Docker container for WebShell - using published ports for Kasm
        ...
        environment = {
            ...
            "KASM_SERVER_HOSTNAME": get_config("webshell:domain", "localhost"),
            "SSL_ENABLED": "true",
            "SECURE_CONNECTION": "true",
            "CONTAINER_MOUNTED": "true",
            "FLAG": f"CTF{{webshell_{container.uuid}}}"  # Add a dummy flag like CTFd-whale does
        }
        ...
        if container.desktop_port > 0:
            ports[6901] = container.desktop_port
            current_app.logger.info(f"[WebShell] Port mapping: 6901->{container.desktop_port}")
        ...
        # Create service in Swarm mode for the WebShell container
        try:
            current_app.logger.info(f"[WebShell] Creating Docker service: {container_name}")
            
            # Create service with published ports (this is different from CTFd-whale)
